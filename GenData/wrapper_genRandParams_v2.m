function wrapper_genRandParams_v2(topology_full_name,topology_short_name,reg_networks,reg_network_idx)
    ...
    original_paramsListFileName = sprintf('paramsList/paramsList%s_regNetwork_%d.mat',topology_full_name,reg_network_idx); 
    original_paramsListFile = load(original_paramsListFileName); 
    original_paramsList = original_paramsListFile.paramsList; 

    for rep = 1:20 
    %     rng(rep) %for replicability control 
        %if branch pathway, we'll need to replace 2 stoich numbers
            % To keep things consistent, let's use the same stoich coeff in
            % round #1 
        if strcmp(topology_short_name,'Branch')
            original_paramsVec = original_paramsList{rep}; 
            bm1 = original_paramsVec(1);
            bm2 = original_paramsVec(2);
            % Sanity Check: bm1 + bm2 = 1 
            if ~isequal(bm1 + bm2,1)
                error('Error with stoichiometry coefficients for the branched pathway')
            end
        ...
        %substrate-level regulation, [0,0.5] for mass-action and a's ) 
        ...
        %parameters
            ...
            for reg_pair_idx = 1:1:size(kineticsMap,1)
                ...
                if stoichMatrix(controller_met,controlled_flux) < 0 
                    temp_parameters_matrix(controlled_flux,controller_met + 1) = 0.5 * rand; %positive index 
    ...
    save(sprintf('paramsList/v2_paramsList%s_regNetwork_%d.mat',topology_full_name,reg_network_idx),'paramsList');
end