...
topology_name_list = {'Branch','UDreg'};
crossTalk_list = {'CrossTalk','NoCrossTalk'};
nT = 100; 
cov = 5; 
missing_metabolite_idx_list_all = {0:1:5,0:1:4};
...
for topo_idx = 2:2%length(topology_name_list)
    ...
    % Preassign cell arrays
        % metadata
    % 6 reg networks * 20 paramSets * # missing metaboltie tested 
    array_size = length(crossTalk_list) * 3 * 20 * length(missing_metabolite_idx_list); 
    ...
    top_structure_array = cell(array_size,1); 
        % Reset count 
    count = 1; 
    for crosstalk_idx = 1:length(crossTalk_list)
    ...
    save(sprintf('test_saved_files/%s_result_struct_more_missing_lower_noise',topology_name),'result_struct'); 
end
...
for topo_idx = 2:2%length(topology_name_list)
    ...
    topology_name = topology_name_list{topo_idx}; 
    missing_metabolite_idx_list = missing_metabolite_idx_list_all{topo_idx}; 
    resultFile = load(sprintf('test_saved_files/%s_result_struct_more_missing_lower_noise.mat',topology_name)); 
    result_struct = resultFile.result_struct;

    % Every #missing metabolite (6/5) rows there should be a unique parameter values  
    fileName_stem = 'paramsList/paramsList';
    correct_struct_length = length(result_struct); 
    row_interval = length(missing_metabolite_idx_list); 
    for row_idx = 1:row_interval:correct_struct_length
        ...
        if isequal(row_idx,1)
            for param_label_idx = 1:length(parameter_labels)
                ...
                eval(sprintf('[result_struct(row_idx:row_idx + row_interval -1).%s] = deal(parameter_values{param_label_idx});',param_label));
            end
        else
            for param_label_idx = 1:length(parameter_labels)
                param_label = parameter_labels{param_label_idx}; 
    ...
    save(sprintf('test_saved_files/%s_result_struct_paramVals_more_missing_lower_noise.mat',topology_name),'result_struct'); 
end
...
for topo_idx = 2:2%length(topology_name_list)
    ...
    missing_metabolite_idx_list = missing_metabolite_idx_list_all{topo_idx}; 
    row_interval = length(missing_metabolite_idx_list); 
    resultFile = load(sprintf('test_saved_files/%s_result_struct_paramVals_more_missing_lower_noise.mat',topology_name));
    ...
    for row_idx = 1:row_interval:length(result_struct)
        % Get parameter sensitivity from result file and assign them in
        % existing result structure 
        sensitivity_idx = ceil(row_idx/row_interval);
        for field_idx = 5:length(sensitivity_fieldNames)
            ...
            eval(sprintf('[result_struct(row_idx:row_idx + row_interval -1).%s] = deal(field_value);',...
                strcat(field_name,'_sensitivity')));
        end
    end
    save(sprintf('test_saved_files/%s_result_struct_param_sensitivity_more_missing_lower_noise.mat',topology_name),'result_struct'); 
end
...
for topo_idx = 2:2%length(topology_name_list)
    ...
    topology_name = topology_name_list{topo_idx}; 
    resultFile = load(sprintf('test_saved_files/%s_result_struct_param_sensitivity_more_missing_lower_noise.mat',topology_name)); 
    result_struct = resultFile.result_struct;
    ...
    save(sprintf('test_saved_files/%s_result_struct_param_ratio_more_missing_lower_noise.mat',topology_name),'result_struct'); 
end
...
for topo_idx = 2:2%length(topology_name_list)
    ...
    topology_name = topology_name_list{topo_idx}; 
    resultFile = load(sprintf('test_saved_files/%s_result_struct_param_ratio_more_missing_lower_noise.mat',topology_name)); 
    result_struct = resultFile.result_struct;
    ...
    time_course_features_struct = filter_result_struct(time_course_features_struct,{'nT','cov'},{100,5}); 
    ...
    save(sprintf('test_saved_files/%s_result_struct_time_course_more_missing_lower_noise',topology_name),'unpacked_result_struct'); 
...
for topo_idx = 2:2%length(topology_name_list)
    topology_name = topology_name_list{topo_idx};
    result_struct_file = load(sprintf('test_saved_files/%s_result_struct_time_course_more_missing_lower_noise.mat',topology_name)); 
    ...
    save(sprintf('test_saved_files/%s_result_struct_time_course_more_missing_lower_noise.mat',topology_name),'result_struct'); 
end
